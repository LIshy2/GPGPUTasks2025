...
void run(int argc, char** argv)
{
    ...
    //
    std::vector<double> load_times(3);
    for (size_t load_try = 0; load_try < 3; ++load_try) {
        timer t;
        input_gpu.writeN(values.data(), n);
        load_times[load_try] = t.elapsed();
    }

    double input_size_gb = sizeof(unsigned int) * n / 1024.0 / 1024.0 / 1024.0;

    std::cout << "______________________________________________________" << std::endl;
    std::cout << "Load to gpu: " << input_size_gb / stats::median(load_times) << " GB/s" << std::endl;
            ...
            if (algorithm == "CPU") {
                ...
                if (context.type() == gpu::Context::TypeOpenCL) {
                    if (algorithm == "01 atomicAdd from each workItem") {
                        ...
                        unsigned int m = div_ceil(n, (unsigned int)GROUP_SIZE);
                        while (m > 1) {
                            ocl_sum04LocalReduction.exec(gpu::WorkSize(GROUP_SIZE, m), reduction_buffer1_gpu, reduction_buffer2_gpu, m);
                            m = div_ceil(m, (unsigned int)GROUP_SIZE);
